...
def upsert_centro(conn, codigo, nombre, tipo="coste", descripcion="", cid=None) -> int:
    with conn:
        if cid:
            conn.execute(
            ...
            return cid
        cur = conn.execute(
            """INSERT INTO centros (codigo, nombre, tipo, descripcion) VALUES (?,?,?,?)
               ON CONFLICT(codigo) DO UPDATE SET nombre=excluded.nombre,
...
# --- Documentos -----------------------------------------------------------
def list_documentos(conn, estado=None, centro_id=None, tipo=None) -> List[Dict[str, Any]]:
...
def insert_documento(conn, **kw) -> int:
    with conn:
        cur = conn.execute(
            """INSERT INTO documentos (tipo, numero, fecha, tercero, concepto, importe,
               cuenta_id, centro_id, estado, notas)
               VALUES (:tipo,:numero,:fecha,:tercero,:concepto,:importe,
                       :cuenta_id,:centro_id,:estado,:notas)""",
            {
...
def update_documento(conn, did, **kw) -> None:
    campos = ["tipo", "numero", "fecha", "tercero", "concepto", "importe",
              "cuenta_id", "centro_id", "estado", "notas"]
    sets, params = [], []
    for c in campos:
        if c in kw:
...
def totales(conn) -> Dict[str, Any]:
    row = conn.execute(
        """SELECT COUNT(*) AS n_docs, COALESCE(SUM(importe),0) AS total,
